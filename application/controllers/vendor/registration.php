...
class registration extends CI_Controller
{
	...
	function index(){
		...
		if($this->registration_model->get_validate_registed_or_not() == 0){
			...
			echo "<script> alert('Vendor sudah melakukan registrasi. Silahkan lanjut untuk submit dokumen dan asset.'); window.location.href='".base_url()."index.php/vendor/registration/registred'; </script>";
		}
	}

	function registred(){
		$this->load->model('models_vendor/registration_model');
		$data['vendor']=$this->registration_model->get_registration_data_vendor();
		$data['pic']=$this->registration_model->get_registration_data_pic();
		$this->load->view('view_vendor/header');
		$this->load->view('view_vendor/registred', $data);
		$this->load->view('view_vendor/footer');
	}
